...
const ShopForm = ({ initialValues }: { initialValues?: Shop }) => {
  ...
  function onSubmit(values: FormValues) {
    ...
    // Construction du payload aligné avec ce que le backend attend
    const payload = {
      ...values,
      logo_image: values.logo_image,                   // ✅ renommer logo → logo_image
      cover_image: values.cover_image,                // déjà au bon format
      website: values.settings?.website ?? null,      // extraire website du settings
      contact: values.settings?.contact ?? null,      // extraire contact du settings
      location: values.settings?.location ?? null,    // extraire location du settings
      address: values.address,                        // garder address
      settings,                                       // conserver settings complet si besoin
      balance: {                                      // idem balance
        ...values.balance,
      },
    };
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        <div className="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8">
          ...
          <Card className="w-full sm:w-8/12 md:w-2/3">
            <FileInput name="logo_image" control={control} multiple={false} />
          </Card>
        </div>
