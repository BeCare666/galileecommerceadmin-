...
export default function Uploader({
  ...
  useEffect(() => {
    const next = getPreviewImage(value);
    setFiles(next);
  }, [value]);
            ...
            onSuccess: (data: any) => {
              ...
              data &&
                data?.map((file: any, idx: any) => {
                  ...
                  const splitArray = file?.url?.split('/');
                  let fileSplitName =
      ...
      {(!!thumbs.length || loading) && (
        <aside className="flex flex-wrap mt-2">
          ...
          {!loading && thumbs.length > 0 && (
            <div className="flex items-center text-green-600 mt-2 ms-2">
              <DoubleCheckIcon className="w-5 h-5" />
              <span className="ml-1 text-sm">Fichier bien chargé</span>
            </div>
          )}
        </aside>
      )}
