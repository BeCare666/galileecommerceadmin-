...
export default function CreateOrUpdateProductForm({
  ...
  const onSubmit = async (values: ProductFormValues) => {
    ...
    if (values.categories && Array.isArray(values.categories)) {
      backendInput.categories = values.categories.map((cat: any) => ({
        categories_id: Number(cat.categories_id),
        sous_categories_id: Array.isArray(cat.sous_categories_id)
          ? cat.sous_categories_id.map(Number)
          : cat.sous_categories_id
            ? [Number(cat.sous_categories_id)]
            : [],
        sub_categories_id: Array.isArray(cat.sub_categories_id)
          ? cat.sub_categories_id.map(Number)
          : cat.sub_categories_id
            ? [Number(cat.sub_categories_id)]
            : [],
      }));
    }
  ...
  if (permission) {
    if (initialValues?.status !== ProductStatus.Draft) {
      // Super admin et produit non Draft => toutes les options
      statusList = [
        {
      ...
      // Produit Draft => seulement Draft / Unpublish
      statusList = [
        {
    ...
    // Pas super admin => uniquement Draft / Unpublish
    statusList = [
      {
        label: 'form:input-label-unpublish-send-for-admin',
        id: 'unpublish',
