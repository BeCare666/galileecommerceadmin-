...
type FormValues = {
  ...
  profile: {
    ...
    bio: string;
    contact: string;
    avatar: Attachment | null; // ✅ vient du Uploader
    notifications: {
      email: string;
      enable: boolean;
    };
...
export default function ProfileUpdate({ me }: any) {
  ...
  // ✅ IMPORTANT : plus d’upload ici — déjà fait par Uploader

  async function onSubmit(values: FormValues) {
    const { name, profile } = values;

    const avatarMapped = profile.avatar
      ? {
        id: profile.avatar.id,
        original: profile.avatar.url,
        thumbnail: profile.avatar.url,
      }
      : undefined; // ✅ PAS null
    ...
    const input: any = {
      ...
      input: {
        ...
        profile: {
          ...
          ...(avatarMapped && { avatar: avatarMapped }), // ✅ clé seulement si présent
        },
      },
    };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      {/* NOTIFICATIONS */}
      {permission && (
        <div className="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8 hidden">
          <Description
